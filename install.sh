...
ZSHRC
    ...
    # Use appropriate P10k config based on environment
    if [ "$ENV_TYPE" = "termux" ]; then
        # Use ASCII-safe config for Termux
        cp "$SCRIPT_DIR/configs/p10k-termux.zsh" "$HOME/.p10k.zsh" 2>/dev/null || \
        curl -fsSL https://raw.githubusercontent.com/akaoio/terminal/main/configs/p10k-termux.zsh -o "$HOME/.p10k.zsh" 2>/dev/null || \
        create_default_p10k_config
    else
        # Use full-featured config for other environments
...
# AKAOIO Terminal - Termux ASCII-safe Prompt
...
() {
  ...
  # Git - Dracula colors, ASCII symbols
